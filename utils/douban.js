...
function getList(type, page, count, tag, search) {
  ...
  if (search) {
    return fetch(type, Object.assign(params, { q: search })).then(
      (res) => res.data
    ); // 响应拦截
  }
  if (tag) {
    return fetch(type, Object.assign(params, { tag: tag })).then(
      (res) => res.data
    ); // 响应拦截
  }
  return fetch(type, params).then((res) => res.data); // 响应拦截
}
...
function getItem(id) {
  ...
  return fetch('subject/' + id).then((res) => res.data); // 响应拦截
}
